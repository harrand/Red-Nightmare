...
namespace game::render
{
	...
	{
	public:
		texture_manager(tz::ren::animation_renderer& anim_ren);
		void lua_initialise(tz::lua::state& state);

		void register_texture(std::string name, std::string_view texture_path);
		tz::ren::animation_renderer::texture_handle get_texture(const std::string& name) const;
		bool has_texture(const std::string& name) const;
	private:
		tz::ren::animation_renderer* anim_ren = nullptr;
		std::unordered_map<std::string, tz::ren::animation_renderer::texture_handle> stored_textures = {};
	};
