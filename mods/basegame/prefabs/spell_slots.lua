rn.mods.basegame.prefabs.spell_slots =
{
	...
	equip_spell = function(uuid, spellname, dont_drop_replacement, slot_override)
		tz.report("equipping spell " .. spellname)
		local slot = rn.spell.spells[spellname].slot

		if slot_override ~= nil then
			slot = slot_override
		end

		-- if we're changing the player's equipped spell, the spell slot display ui needs to be updated:
		if rn.mods.basegame.prefabs.spell_slots.would_replace_on_equip(uuid, spellname, slot) and (uuid == rn.player.get()) then
			rn.player_spell_slot_override(spellname, slot_override)
		end
		...
		if dont_drop_replacement == nil and existing_spellname ~= nil then
	...
	would_replace_on_equip = function(uuid, spellname, slot_override)
		local slot = rn.spell.spells[spellname].slot
		if slot_override ~= nil then
			slot = slot_override
		end
