...
rn.mods.basegame2.prefabs.model_humanoid =
{
	...
	update = function(uuid, delta_seconds)
		...
		local should_stop_move_animation = (currently_playing == rn.entity.prefabs.model_humanoid.get_run_animation(uuid)) and not moving
		...
		elseif currently_playing == nil then
			sc:entity_play_animation(uuid, rn.entity.prefabs.model_humanoid.get_idle_animation(uuid))
		end
	end,
	...
	on_move = function(uuid, xdiff, ydiff, zdiff)
		local sc = rn.current_scene()
		local currently_playing = sc:entity_get_playing_animation(uuid)
		local movement_anim_multiplier = 1.1 * rn.entity.prefabs.combat_stats.get_movement_speed(uuid) / rn.entity.prefabs.model_humanoid.default_movement_speed
		local anim = rn.entity.prefabs.model_humanoid.get_run_animation(uuid)
	...
	on_stop_moving = function(uuid)
		if not rn.spell.is_casting(uuid) then
			rn.current_scene():entity_play_animation(uuid, rn.entity.prefabs.model_humanoid.get_idle_animation(uuid))
		end
	end,
	on_cast_begin = function(uuid, spellname)
		...
		rn.entity.prefabs.model_humanoid.play_animation(uuid, cast_anim, false, animation_duration / cast_duration)
	end,
	on_death = function(uuid, dmg, magic_type, enemy_uuid)
		rn.current_scene():entity_play_animation(uuid, "ZombieDeath")
		if rn.entity.prefabs.model_humanoid.drop_items_on_death(uuid) then
	...
	on_equip = function(uuid, itemname)
		...
		local subobject = rn.entity.prefabs.model_humanoid.item_slot_to_subobject(itemdata.slot)
		if (itemdata.slot ~= rn.item.slot.left_hand) and (itemdata.slot ~= rn.item.slot.right_hand) then
			-- armour
			rn.entity.prefabs.model_humanoid.set_subobject_visible(uuid, subobject, true)
			local col = itemdata.colour or {1.0, 1.0, 1.0}
			rn.entity.prefabs.model_humanoid.set_subobject_colour(uuid, subobject, col[1], col[2], col[3])
			rn.entity.prefabs.model_humanoid.set_subobject_texture(uuid, subobject, itemdata.texture)
			if itemdata.normal_map ~= nil then
				rn.entity.prefabs.model_humanoid.set_subobject_normal_map(uuid, subobject, itemdata.normal_map)
			end
			if itemdata.emissive_map ~= nil then
				rn.entity.prefabs.model_humanoid.set_subobject_emissive_map(uuid, subobject, itemdata.emissive_map)
				local tint = itemdata.emissive_tint;
				if tint ~= nil then
					rn.entity.prefabs.model_humanoid.set_subobject_emissive_tint(uuid, subobject, tint[1], tint[2], tint[3])
				end
			end
		else
			-- weapon
			...
			if itemdata.texture ~= nil then
				rn.entity.prefabs.model_humanoid.set_subobject_texture(weapon_entity, 2, itemdata.texture)
			end
			if itemdata.emissive_map ~= nil then
				rn.entity.prefabs.model_humanoid.set_subobject_emissive_map(weapon_entity, 2, itemdata.emissive_map)
				if itemdata.emissive_tint ~= nil then
					local tint = itemdata.emissive_tint
					rn.entity.prefabs.model_humanoid.set_subobject_emissive_tint(weapon_entity, 2, tint[1], tint[2], tint[3])
				end
			end
	...
	on_unequip = function(uuid, itemname)
		...
		if (itemdata.slot ~= rn.item.slot.left_hand) and (itemdata.slot ~= rn.item.slot.right_hand) then
			-- armour
			rn.entity.prefabs.model_humanoid.set_subobject_visible(uuid, subobject, false)
		else
			-- weapon
	...
	set_visible = function(uuid, visible)
		rn.entity.prefabs.model_humanoid.set_subobject_visible(uuid, base_subobj, visible)	
	end,
	set_texture = function(uuid, texname)
		rn.entity.prefabs.model_humanoid.set_subobject_texture(uuid, base_subobj, texname)
	end,
	...
	set_colour = function(uuid, r, g, b)
		rn.entity.prefabs.model_humanoid.set_subobject_colour(uuid, base_subobj, r, g, b)
	end,
	...
	face_direction = function(uuid, dx, dy)
		...
		if math.abs(dx) > math.abs(dy) then
			...
			rn.entity.prefabs.model_humanoid.set_rotation(uuid, 0.0, math.max(-signy, 0.0) * math.pi, math.pi * signy * tilt_factor)
		end
	end,
